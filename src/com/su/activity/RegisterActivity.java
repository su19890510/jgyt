	...
	private void setListener() {
		...
		confirm.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				{
					Toast.makeText(register, "名字不能为空",
							Toast.LENGTH_SHORT).show();
					return;
				}
				else if(name.trim().length() < 4 || name.trim().length() > 12)
				{
					Toast.makeText(register, "用户名长度不符合",
							Toast.LENGTH_SHORT).show();
					return;
				}
				...
				{
					Toast.makeText(register, "手机号不能为空",
							Toast.LENGTH_SHORT).show();
					return;
				}
				else if(phone.trim().length() != 11 )
				{
					Toast.makeText(register, "请输入11位手机号",
							Toast.LENGTH_SHORT).show();
					return;
				}
				...
				{
					Toast.makeText(register, "密码不能为空",
							Toast.LENGTH_SHORT).show();
					return;
				}
				else if(password.trim().length() < 4 || password.trim().length() > 6)
				{
					Toast.makeText(register, "密码长度不符合要求",
							Toast.LENGTH_SHORT).show();
					return;
				}
				...
				JSONObject get = bb.sendHttpRequest("account/register", tt, 0);
				if(get == null)
				{   
				...
				try {
					...
					if(code != 200)
					{
						Toast.makeText(register, get.getString("message"),
								Toast.LENGTH_SHORT).show();
						return;
					}
